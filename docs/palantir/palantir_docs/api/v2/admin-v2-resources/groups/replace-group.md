...
| INVALID_ARGUMENT | `AttributesNotEditable` | One or more attributes are not editable. Attributes prefixed with "multipass:" are reserved for internal use by Foundry and are not editable. |
| PERMISSION_DENIED | `ReplaceGroupPermissionDenied` | Could not replace the Group. |
