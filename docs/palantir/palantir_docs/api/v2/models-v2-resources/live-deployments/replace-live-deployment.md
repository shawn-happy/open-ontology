...
| CONFLICT | `LiveDeploymentSourcesCannotBeUpdatedWhileRunning` | Sources can only be updated when the live deployment is disabled. |
...
| INVALID_ARGUMENT | `UnsupportedLiveDeployment` | The Live Deployment type is not supported by the API. |
| PERMISSION_DENIED | `ReplaceLiveDeploymentPermissionDenied` | Could not replace the LiveDeployment. |
