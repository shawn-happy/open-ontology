...
| INVALID_ARGUMENT | `ViewPrimaryKeyMustContainAtLeastOneColumn` | No columns were provided as part of the primary key |
| INVALID_ARGUMENT | `ViewPrimaryKeyRequiresBackingDatasets` | Cannot add a primary key to a View that does not have any backing datasets. |
| INTERNAL | `ViewDatasetCleanupFailed` | Failed to delete dataset following View creation failure. |
...
| INVALID_ARGUMENT | `InvalidViewPrimaryKeyColumnType` | The type of each referenced column in the primary key must be one of the following: BYTE, SHORT, DECIMAL,<br>INTEGER, LONG, STRING, BOOLEAN, TIMESTAMP or DATE. |
| INVALID_ARGUMENT | `InvalidViewPrimaryKeyDeletionColumn` | The deletion column must be a boolean. |
| INVALID_ARGUMENT | `ViewPrimaryKeyDeletionColumnNotInDatasetSchema` | The deletion column is not present in the dataset. |
| INVALID_ARGUMENT | `NotAllColumnsInPrimaryKeyArePresent` | Not all columns in the View's primary key are present in the dataset(s). |
...
| PERMISSION_DENIED | `NotAuthorizedToDeclassifyMarkings` | The caller does not have DECLASSIFY permission on these markings or the markings do not exist. |
| PERMISSION_DENIED | `CreateViewPermissionDenied` | Could not create the View. |
