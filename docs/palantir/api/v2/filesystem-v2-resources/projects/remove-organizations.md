...
| INVALID_ARGUMENT | `OrganizationCannotBeRemoved` | An organization cannot be removed from a project if it would result in a project with no organizations<br>under a space marked with an organization. |
...
| PERMISSION_DENIED | `RemoveOrganizationsPermissionDenied` | Could not removeOrganizations the Project. |
