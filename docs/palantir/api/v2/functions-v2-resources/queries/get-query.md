...
| `latestVersionResolution` | enum | 否 | Controls how latest version is resolved when `version` is omitted. Defaults to `SEMANTIC_VERSION`.<br>示例: `PUBLISH_TIME` |
| `includePrerelease` | boolean | 否 | When resolving the latest version, whether prerelease versions are considered. Defaults to `false`,<br>except when `latestVersionResolution` is `PUBLISH_TIME`. Not supported together with `version`. |
...
| 错误码 | 错误名称 | 说明 |
| --- | --- | --- |
| NOT_FOUND | `QueryNotFound` | The given Query could not be found. |
