...
| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `FileImport` | object | 是 | The created FileImport<br>示例: `{"datasetRid":"ri.foundry.main.dataset.c26f11c8-cdb3-4f44-9f5d-9816ea1c82da","importMode":"SNAPSHOT","displayName":"My file import","connectionRid":"ri.magritte..source.c078b71b-92f9-41b6-b0df-3760f411120b","branchName":"master","subfolder":"subfolder1/subfolder2","rid":"ri.magritte..extract.27bb4f2b-63b8-44b8-a579-4e2bd65ba158","fileImportFilters":[{"type":"pathMatchesFilter","regex":"my-subfolder"}]}` |
...
| INVALID_ARGUMENT | `FileSizeFilterMissingGreaterThanAndLessThan` | Both the `gt` and `lt` properties are missing from the FileSizeFilter. At least one of these<br>properties must be present |
| INVALID_ARGUMENT | `FileSizeFilterGreaterThanCannotBeNegative` | The `gt` property in the FileSizeFilter cannot be a negative number. |
| INVALID_ARGUMENT | `FileSizeFilterLessThanMustBeOneByteOrLarger` | The `lt` property in the FileSizeFilter must be at least 1 byte. |
| INVALID_ARGUMENT | `FileSizeFilterInvalidGreaterThanAndLessThanRange` | The provided `gt` and `lt` properties in the FileSizeFilter are invalid. No files will ever<br>satisfy the provided range. The value specified for `gt` must be strictly less than `lt - 1`. |
| INVALID_ARGUMENT | `FileAtLeastCountFilterInvalidMinCount` | The provided `minFilesCount` property in the FileAtLeastCountFilter must be strictly greater than 0. |
| INVALID_ARGUMENT | `FileImportCustomFilterCannotBeUsedToCreateOrUpdateFileImports` | Custom file import filters can be fetched but cannot currently be used<br>when creating or updating file imports. |
| INVALID_ARGUMENT | `FileImportNotSupportedForConnection` | The specified connection does not support file imports. |
| NOT_FOUND | `DatasetNotFound` | The requested dataset could not be found, or the client token does not have access to it. |
| PERMISSION_DENIED | `CreateFileImportPermissionDenied` | Could not create the FileImport. |
